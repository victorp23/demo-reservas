  ...
  return <section className="court-catalog" id="canchas">
    ...
    <div className="court-grid">
      {courts.map((court) => <article className="court-card" key={court.id}>
        ...
        <div className="court-card-content"><p className="court-type">{formatType(court.type)}</p><h3>{court.name}</h3><div className="court-meta"><span><Clock3 size={15} /> Reservas de {court.duration || 60} min</span>{court.capacity && <span><UsersRound size={15} /> {court.capacity} jugadores</span>}</div>{court.price && <p className="court-price">{court.price}</p>}<a className="court-availability-button" href={`/horarios?cancha=${court.id}`}>Consultar horarios</a></div>
      </article>)}
    </div>
  </section>
}
